...
import app.kotlin.devpbleite.barbershop.databinding.ActivityAgendamentoBinding
...
class Agendamento : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding.btAgendar.setOnClickListener {
      ...
      when {
        hora.isEmpty() -> {
          mensagem(it, "Selecione um horário!", "#FF0000")
        }
        hora < "08:00" && hora > "19:00" -> {
          mensagem(it, "BarberShop está fechado. Horário de atendimento das 08:00 às 19:00.", "#FF0000")
        ...
        barbeiro1.isChecked && data.isNotEmpty() && hora.isNotEmpty() -> {
          salvarAgendamento(it, nome, "Chico Barber", data, hora)
        }
        barbeiro2.isChecked && data.isNotEmpty() && hora.isNotEmpty() -> {
          salvarAgendamento(it, nome, "Nélio José", data, hora)
        }
        barbeiro3.isChecked && data.isNotEmpty() && hora.isNotEmpty() -> {
          salvarAgendamento(it, nome, "Jailson Marques", data, hora)
  
  
        }
        else -> {
          mensagem(it, "Selecione um barbeiro!", "#FF0000")
